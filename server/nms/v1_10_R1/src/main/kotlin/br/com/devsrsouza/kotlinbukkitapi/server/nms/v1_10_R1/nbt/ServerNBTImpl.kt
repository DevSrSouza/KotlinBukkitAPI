package br.com.devsrsouza.kotlinbukkitapi.server.nms.v1_10_R1.nbt
...
object ServerNBTImpl : ServerNBT<NBTTagCompound, NBTBase> {
    ...
    override fun serverTagAsKotlinNBTTag(tag: NBTBase): ITag {
        return when(tag) {
            is NBTTagString -> StringTag(tag.c_())
            is NBTTagDouble -> DoubleTag(tag.h())
            ...
            is NBTTagList -> {
                val typeId = if(tag.isEmpty) TagType.END else TagType.byID(tag.h(0).getTypeId().toInt())!!
